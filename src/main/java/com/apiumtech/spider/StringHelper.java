...
{
	static private HashMap<String, Integer> htmlEntities = new HashMap<String, Integer>(); 
	{
	...
	{
        // support for %XX format
        s = s.replaceAll("%([0-9A-Fa-f]{2,2})", "&#x$1;");
	...
	static String encode(String text, String charsetName)
	{
		String result = "";
	...
	public static String cleanHtmlText(String text)
	{
		...
		r = text.replaceAll("[\\r\\n]+", " "); // lines
		r = r.replaceAll("<[^<>]*>", " "); // tags
		r = r.replaceAll("<[^<>]*>", " "); // tags again
		r = r.replaceAll("[\\t]+", " "); // tabs
		r = r.replaceAll("&[^;]*;", " "); // &?;
		r = r.replaceAll(" [ ]+", " "); // > 1 spaces
		return r.trim();
	}
}
